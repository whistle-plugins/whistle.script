...
const parseArguments = (args) => {
  ...
  if (typeof options.proxyHost !== 'string') {
    delete options.proxyHost;
    delete options.proxyPort;
  } else if (!(options.proxyPort > 0)) {
    delete options.proxyPort;
    const proxyOptions = parseHost(options.proxyHost);
    if (proxyOptions) {
      options.proxyHost = proxyOptions.host;
      options.proxyPort = proxyOptions.port;
    }
  }
  return options;
};
...
exports.request = (ctx, opts) => {
  ...
  let r = request;
  const { proxyHost, proxyPort } = opts;
  if (proxyHost) {
    r = request.defaults({
      proxy: `http://${proxyHost}${proxyPort ? `:${proxyPort}` : ''}`,
    });
